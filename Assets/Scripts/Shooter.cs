...
public class Shooter : MonoBehaviour {
			...
			if (projectile)
			{
				...
				if (shootSFX)
				{
					if (newProjectile.GetComponent<AudioSource> ()) { // the projectile has an AudioSource component
						...
						if(this.is_sound) newProjectile.GetComponent<AudioSource> ().PlayOneShot (shootSFX);
					} else {
						// dynamically create a new gameObject with an AudioSource
						// this automatically destroys itself once the audio is done
						if (this.is_sound) AudioSource.PlayClipAtPoint (shootSFX, newProjectile.transform.position);
					}
				}
			}
		}
	...
	public void set_sound(bool is_sounds)
    {
		this.is_sound = is_sounds;
    }
}
